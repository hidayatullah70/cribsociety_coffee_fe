...
export const GuestDashboard: React.FC = () => {
  ...
  return (
    <div className="flex-1 p-4 sm:p-6 lg:p-8 space-y-8 bg-brand-black text-brand-white overflow-y-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-brand-white/60 font-medium flex items-center gap-1.5">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link to="/pos">
            <Button size="sm" variant="primary" className="font-bold flex items-center gap-2 shadow-md shadow-brand-red/20">
      ...
      <Card className="p-5 sm:p-6 bg-brand-black-card border border-brand-black-muted shadow-2xl">
        <div className="max-w-xl">
          ...
          <form onSubmit={handleTrackOrder} className="flex gap-2 mt-4">
            ...
            <Button type="submit" variant="primary" className="font-bold px-5 shadow-md shadow-brand-red/20">
              Track
            </Button>
          </form>
          ...
          {trackedOrder && (
            <div className="mt-4 p-4 rounded-xl bg-brand-black-soft border-2 border-brand-red/50 animate-in fade-in duration-200">
              <div className="flex items-center justify-between">
                <div>
                  <span className="font-mono font-black text-lg text-white">
                    #{trackedOrder.orderNumber}
                  </span>
                  <span className="text-xs text-brand-white/70 ml-2 font-semibold">
              ...
              <div className="mt-2 text-xs text-brand-white/80 space-y-1">
                {trackedOrder.items.map((i, idx) => (
                  <div key={idx} className="flex justify-between">
                    <span>
                      {i.quantity}x {i.name} {i.variantName ? `(${i.variantName})` : ''}
                    </span>
                    <span className="font-mono font-bold text-white">{formatIDR(i.itemTotal)}</span>
                  </div>
                ))}
              </div>

              <div className="mt-3 pt-2 border-t border-brand-black-muted flex items-center justify-between text-xs">
                <span className="font-bold text-white font-mono">Total Paid: {formatIDR(trackedOrder.total)}</span>
                <span className="text-brand-white/50 font-mono">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card className="p-6 border border-brand-black-muted bg-brand-black-card space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-brand-black-muted">
            <div className="flex items-center gap-2.5">
              <div className="w-10 h-10 rounded-xl bg-brand-red-soft text-brand-red flex items-center justify-center font-bold border border-brand-red/30">
                <Flame className="w-6 h-6 animate-pulse text-brand-red" />
              </div>
              <div>
                <h3 className="font-black text-lg text-white">NOW BREWING</h3>
                <span className="text-xs text-brand-white/60 font-semibold">In preparation by Baristas</span>
              </div>
            </div>
          ...
          {isLoading ? (
            ...
            <div className="p-8 text-center text-xs text-brand-white/50">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {preparingOrders.map((order) => (
                <div
                  key={order.id}
                  className="p-4 rounded-xl bg-brand-black-soft border border-brand-black-muted flex flex-col justify-between"
                >
                  <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-bold text-brand-white/80 truncate mt-1">
                    {order.customerName || 'Counter'}
                  </span>
                  <span className="text-[11px] text-brand-white/60 mt-1">
        ...
        <Card className="p-6 border border-utility-success/40 bg-brand-black-card space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-brand-black-muted">
            <div className="flex items-center gap-2.5">
              <div className="w-10 h-10 rounded-xl bg-utility-success-soft text-utility-success flex items-center justify-center font-bold border border-utility-success/30">
                <CheckCircle2 className="w-6 h-6 text-utility-success" />
              </div>
              <div>
                <h3 className="font-black text-lg text-white">READY FOR PICKUP</h3>
                <span className="text-xs text-brand-white/60 font-semibold">Please collect at the counter bar</span>
              </div>
            </div>
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {readyOrders.map((order) => (
                <div
                  key={order.id}
                  className="p-4 rounded-xl bg-utility-success-soft border border-utility-success/40 flex flex-col justify-between"
                >
                  <div className="flex items-center justify-between">
      ...
      <Card className="p-6 space-y-5 bg-brand-black-card border-brand-black-muted">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-3 border-b border-brand-black-muted gap-2">
          <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {products.slice(0, 8).map((product) => (
            <div
              key={product.id}
              className="p-4 rounded-xl bg-brand-black-soft border border-brand-black-muted flex flex-col justify-between space-y-2"
            >
              <div>
                <div className="flex items-center justify-between mb-1">
                  <Badge variant={product.available ? 'brand' : 'danger'} size="sm">
                ...
                <p className="text-[11px] text-brand-white/60 line-clamp-2 mt-1">
                  {product.description}
                </p>
              </div>

              <div className="pt-2 border-t border-brand-black-muted text-[10px] text-brand-white/50 flex justify-between items-center">
